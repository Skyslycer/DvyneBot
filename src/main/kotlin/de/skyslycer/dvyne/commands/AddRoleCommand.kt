    ...
    fun onGuildMessageReceived(event: GuildMessageReceivedEvent, isBotMention: Boolean) {
        ...
        if (args.count() != 4) {
            val wrongUsageEmbed = EmbedBuilder()
                ...
                .setDescription("Please use `$prefix addrole @User @Role`!")
                .setFooter(event.author.asTag)
                .setTimestamp(Instant.from(ZonedDateTime.now()))
                .build()
        ...
        try {
            ...
            val successfulEmbed = EmbedBuilder()
                ...
                .setDescription("Successfully added the role ${mentionedRole.asMention} to the member ${mentionedUser.asMention}")
                .setFooter(event.author.asTag)
                .setTimestamp(Instant.from(ZonedDateTime.now()))
                .build()
